...
class CRDTFactory {
   ...
   */
  public registerType<R, T extends AutoCouchCRDT<R>>(
    typeName: string,
    createFunction: (...parameters: any) => T,
    loadFunction: (doc: Automerge.Doc<R>) => T,
  ) {
  ...
  public createObject<R, T extends AutoCouchCRDT<R>>(typeName: string, ...parameters: any): T {
    const create = this.createFunctions.get(typeName);
    if (create) {
      return create(...parameters) as T;
    } else {
  ...
  public async loadObject<R, T extends AutoCouchCRDT<R>>(objectId: string): Promise<T> {
    return db.get(objectId)
      .then((doc: any) => {
        const automergeDoc: Automerge.Doc<AutoCouchObject<R>> = Automerge.applyChanges(Automerge.init(), doc.changes);
        const typeName = automergeDoc.objectType;
        const load = this.loadFunctions.get(typeName);
        if (load) {
          return load(automergeDoc) as T;
        } else {
          throw new Error('LoadFunction not defined for ' + typeName);
        }
      })
      .catch(err => {
        console.error("Error in loadObject: %s", err);
        return Promise.reject(err);
      });
  }
}

export const factory = new CRDTFactory();
