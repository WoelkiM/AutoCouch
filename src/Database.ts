...
class Database {
  ...
  constructor(localDB: string, globalDB: string) {
    this.localpouchDB = new PouchDB(localDB);
    this.globalpouchDB = new PouchDB(globalDB);
  ...
  ): Promise<PouchDB.Core.IdMeta & PouchDB.Core.GetMeta> {
...
switch (process.env.NODE_ENV) {
  case 'test':
    ...
    break;
  default:
    const localEnv = process.env.AUTOCOUCH_LOCAL_DB;
    const globalEnv = process.env.AUTOCOUCH_GLOBAL_DB;
    if (!localEnv) {
      console.warn(
        "Location of local PouchDB not set, using 'localPouchDB'. Set the variable AUTOCOUCH_LOCAL_DB to use your own location.",
      );
      local = 'localPouchDB';
    }
    if (!globalEnv) {
      console.warn(
        "Address of global CouchDB not set, using local fallback 'globalPouchDB'. Set the variable AUTOCOUCH_GLOBAL_DB to use your own address.",
      );
      global = 'globalPouchDB';
    }
}

export const db = new Database(local, global);
