...
export abstract class AutoCouchCRDT<T> {
  ...
  constructor(objectType: string, objectId: string, object: T, automergeDoc?: Automerge.Doc<AutoCouchObject<T>>) {
    if (automergeDoc) {
      ...
      const id = objectId ? objectId : uuid.v4();
    ...
    try {
      ...
      console.warn(
        'Object does already exist in the registry. Ignore this if the object was created by loading from the registry.',
      );
    }
    db.get(this.getObjectId())
      .then((result) => this.mergeFunction(result))
      .catch(standardCatch("constructor:get"));
    this.syncFunction();
    this.handlers = [];
  }
   ...
   */
  protected async change(changeFn: (obj: T) => void) {
  ...
  protected getHandlers(): ((data?: any) => void)[] {
  ...
  private conflictFunction(pouchDoc: any, obj: any): void {
    ...
    if (pouchDoc._conflicts) {
      for (const con of pouchDoc._conflicts) {
        try {
          db.get(this.getObjectId(), { rev: con })
            .then((pouchObject) => this.mergeFunction(pouchObject))
            .catch((_err) => {
              obj.changeFunction();
            });
  ...
  private update() {
    const changes = Automerge.getChanges(Automerge.init(), this.automergeDoc);
    const myDoc = {
